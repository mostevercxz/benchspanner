...
func buildVertexUpdateMutation(uid, attr88, attr89, attr90, attr91, attr92 int64) *spanner.Mutation {
	// Calculate shard for this UID
	shard := calcShard(uid)

	// Update specific attributes - need to include key columns for the update
	columns := []string{"shard", "uid", "attr88", "attr89", "attr90", "attr91", "attr92"}
	values := []interface{}{shard, uid, attr88, attr89, attr90, attr91, attr92}
...
func main() {
	...
	case "all":
		...
		// Test vertex update performance
		spannerUpdateVertexTest(client)
	...
	default:
		...
		log.Println("Available test types: setup, setupindex, write-vertex, write-edge, update-vertex, update-edge, relation, read-invert, all")
		os.Exit(1)
	}
...
// spannerReadInvertTest performs invert read testing with multiple goroutines
